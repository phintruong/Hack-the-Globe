...
export default function LivePage() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f8fbff] text-black overflow-hidden select-none"
         style={{ fontFamily: "'Google Sans', 'Roboto', Arial, sans-serif" }}>

      {/* ── Top bar ── */}
      <header className="h-16 flex items-center justify-between px-6 shrink-0 border-b border-[#caf0f8]">
        {/* Left: meeting info */}
        <div className="flex items-center gap-4 min-w-0">
          <span className="text-[22px] font-normal text-black tracking-[-0.01em] truncate">
            UniVoice Interview
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {currentLetter && (
            <span className="flex items-center gap-1.5 bg-[#caf0f8] px-3 py-1.5 rounded-full">
              <span className="text-base font-medium font-mono text-black">{currentLetter}</span>
      ...
      <div className="flex flex-1 min-h-0 px-2 pb-0">
        ...
        <div className="flex-1 flex items-center justify-center p-2 min-h-0">
          <div className={`w-full h-full grid gap-3 ${
            ...
            {/* Tile: Interviewer */}
            <div className="relative bg-[#ade8f4] rounded-xl overflow-hidden group transition-all duration-200">
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <div className="w-20 h-20 rounded-full bg-[#023e8a] flex items-center justify-center mb-3 shadow-lg">
                  <span className="text-3xl font-normal text-white">I</span>
                </div>
                <span className="text-sm text-black font-medium">Interviewer</span>
              </div>
              {/* Caption overlay */}
              {listening && interimText && (
                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#03045e]/80 to-transparent pt-8 pb-4 px-4">
              ...
              <div className="absolute bottom-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <span className="text-xs text-black bg-white/80 px-2 py-1 rounded">
                  Interviewer
                </span>
              </div>
            ...
            <div className="relative bg-[#ade8f4] rounded-xl overflow-hidden group transition-all duration-200">
              {camOn ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <div className="w-20 h-20 rounded-full bg-[#0077b6] flex items-center justify-center mb-3 shadow-lg">
                    <span className="text-3xl font-normal text-white">Y</span>
                  </div>
                  <span className="text-sm text-black font-medium">You</span>
                </div>
              )}
              {/* Detected letter badge */}
              {currentLetter && (
                <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-lg">
                  <span className="text-2xl font-medium text-black">{currentLetter}</span>
                </div>
              )}
              {/* Name label */}
              <div className="absolute bottom-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <span className="text-xs text-black bg-white/80 px-2 py-1 rounded">You</span>
              </div>
              {/* Mic off indicator */}
              {!micOn && (
            ...
            <div className="relative bg-[#ade8f4] rounded-xl overflow-hidden group transition-all duration-200">
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <div className="w-20 h-20 rounded-full bg-[#0096c7] flex items-center justify-center mb-3 shadow-lg">
                  <span className="text-3xl font-normal text-white">P</span>
                </div>
                <span className="text-sm text-black font-medium">Panel Member</span>
              </div>
              <div className="absolute bottom-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <span className="text-xs text-black bg-white/80 px-2 py-1 rounded">
                  Panel Member
        ...
        {/* ── Side panel ── */}
        {sidePanel && (
          <div className="w-[360px] shrink-0 bg-white flex flex-col min-h-0 ml-2 rounded-xl border border-[#caf0f8] overflow-hidden transition-all duration-300 shadow-sm">
            {/* Panel header */}
            <div className="h-14 flex items-center justify-between px-5 border-b border-[#caf0f8] shrink-0">
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setSidePanel("chat")}
                  className={`px-4 py-1.5 text-sm rounded-full transition-colors ${
                    sidePanel === "chat"
                      ? "bg-[#caf0f8] text-[#0077b6] font-medium"
                      : "text-black/60 hover:bg-[#caf0f8]/50"
                  }`}
                >
                  Messages
                </button>
              ...
              <button
                ...
                className="w-8 h-8 rounded-full hover:bg-[#caf0f8] flex items-center justify-center transition-colors"
              >
            ...
            {sidePanel === "chat" && (
              <>
                {/* Quick replies */}
                <div className="px-4 py-3 border-b border-[#caf0f8] shrink-0">
                  <div className="flex flex-wrap gap-1.5">
                    {["Yes", "No", "Could you repeat?", "Thank you", "I need a moment"].map((phrase) => (
                      <button
                        ...
                        disabled={!connected}
                        className="text-xs bg-transparent border border-[#ade8f4] text-black px-3 py-1.5 rounded-full
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 min-h-0">
                  {chatMessages.length === 0 && !interimText && (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      <svg width="48" height="48" viewBox="0 0 24 24" fill="#ade8f4" className="mb-4">
                        <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z" />
                      </svg>
                      <p className="text-sm text-black/60 mb-1">No messages yet</p>
                  ...
                  {chatMessages.map((msg, i) => (
                    <div key={i}>
                      ...
                      <p className="text-sm text-black/80 leading-relaxed pl-0">{msg.text}</p>
                    </div>
                  ))}
                  {interimText && (
                    <div>
                      <div className="flex items-baseline gap-2 mb-0.5">
                        <span className="text-xs font-medium text-black">Interviewer</span>
                        <span className="text-[11px] text-black/40">typing</span>
                      </div>
                      <p className="text-sm text-black/60 italic leading-relaxed">{interimText}…</p>
                    </div>
                  )}
      ...
      {/* ── Bottom control bar ── */}
      <div className="h-20 flex items-center justify-between px-6 shrink-0 border-t border-[#caf0f8]">
        {/* Left: meeting info */}
        <div className="flex items-center gap-3 w-52">
          <span className="text-sm text-black/60">{clock}</span>
          <span className="text-[#ade8f4]">|</span>
          <span className="text-xs text-black/60 truncate">univoice-interview</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-150 ${
              micOn
                ? "bg-[#caf0f8] hover:bg-[#ade8f4] hover:shadow-md"
                : "bg-[#ea4335] hover:bg-[#d33828] hover:shadow-md"
            }`}
            title={micOn ? "Turn off microphone" : "Turn on microphone"}
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill={micOn ? "#03045e" : "white"}>
              {micOn ? (
          ...
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill={camOn ? "#03045e" : "white"}>
              {camOn ? (
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-150 ${
              listening
                ? "bg-[#0077b6] hover:bg-[#023e8a] hover:shadow-md"
                : "bg-[#caf0f8] hover:bg-[#ade8f4] hover:shadow-md"
            } disabled:opacity-40 disabled:cursor-not-allowed`}
            title={listening ? "Turn off captions" : "Turn on captions"}
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill={listening ? "white" : "#03045e"}>
          ...
          <button
            ...
            title="Sign input"
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill={sidePanel === "sign" ? "white" : "#03045e"}>
          ...
          {/* Divider */}
          <div className="w-px h-8 bg-[#caf0f8] mx-1" />
        ...
        <div className="flex items-center gap-2 w-52 justify-end">
          {/* People */}
          <button
            className="w-12 h-12 rounded-full bg-transparent hover:bg-[#caf0f8] flex items-center justify-center transition-colors"
            title="People"
          >
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
              sidePanel === "chat" ? "bg-[#caf0f8]" : "hover:bg-[#caf0f8]"
            }`}
            title="Chat with everyone"
          >
            <svg width="22" height="22" viewBox="0 0 24 24" fill={sidePanel === "chat" ? "#0077b6" : "#03045e"}>
              <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z" />
            </svg>
          </button>
